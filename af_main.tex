...
The optimal weight for a given activation function $\sigma$ is the minimizer of $\ell_2(\underline{y}, \bm{x})$.



For different problems, we suppose $\underline{y}, \bm{x}$ are drawn from distribution $G$.
That is, $\underline{y}, \bm{x}$ are samples of random vector $\underline{Y}$ and random matrix $\bm{X}$.
...
we can decrease $\mathrm{E}[\sigma]$ in the rate of $\abs{\mathrm{C}[\xi]}$ along the perturbation direction $\xi$.
...
In this form, we can see more clearly that $\mathrm{C}[\xi]$ is the coefficient of the second order term of $\mathrm{E}[\sigma]$.
