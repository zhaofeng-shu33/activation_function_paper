...
enhance the expressive power of the neural network.
...
Another shortcoming of this approach is that the searched activation function does not generalize for other problems.
In addition to empirical study, there are also theoretical investigations which study specific network architecture. For example, \cite{AroraBMM18} focuses on studying the number of layers and hidden units while choosing ReLU activation function. Such researches are restricted on a specific activation function and seldom explore about the theoretical property of activity function, which is important to the expressive power of neural network. Our study fills this gap by studying how the activation function reduces the neural network loss. 

To be specific, we fix the network structure and choose activation function from differential function space.
...
To simplify the analysis, we consider the one-node neural network which outputs one-dimensional feature. This one-node network can be regarded as a special ResNet model, which learns the residual representation of feature space.
% This network can be regarded as a kind of non-linear regression.
In addition, we assume input features are orthogonal to each other since orthogonality can simply be achieved by PCA (Principal Component Analysis).
To achieve universally optimal solution, we use an averaged error to measure the network loss.
We find that the averaged loss is minimized when the activation function is Hermite orthogonal polynomial, which
has been argued to be beneficial as activation function in previous work \cite{ma2005constructive}.
...
we choose $\bm{X}$ as uniformly distributed random orthogonal matrix \cite{eaton1989group}. The choice of $\bm{X}$ is based on the following observations: If each element of $\bm{X}'$ is i.i.d. $\mathcal{N}(0, 1)$ random variables and $\sigma(\bm{X}'\underline{w}')$ is considered, we can make the transformation $\bm{X} = \bm{X}'(\bm{X}'^\T\bm{X}')^{-1/2}, \underline{w} = (\bm{X}'^\T\bm{X}')^{1/2}\underline{w}'$. We see that $\bm{X}'\underline{w}' = \bm{X}\underline{w}$. From \cite[Proposition 7.1]{eaton1989group} $\bm{X}$ is a random orthogonal matrix.
