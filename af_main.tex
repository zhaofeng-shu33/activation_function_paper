...
For example, to obtain better representation of features, it is possible to optimize over parameterized activation function \cite{ramachandran2017searching}.
...
In addition to empirical study, there are also theoretical investigation on the expressive power of neural networks, which mainly focuses on studying the number of layers and hidden units while choosing ReLU activation function \cite{AroraBMM18}. However, current research seldom explores about the theoretical property of activity function, which is important to the expressive power of neural network. Our study fills this gap by studying how the activation function reduces the neural network loss. 
...
We find that the averaged loss is minimized when the non-linear term is Hermite orthogonal polynomial, which
