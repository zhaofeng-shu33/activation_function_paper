...
In addition, the searched activation function may not behave well for all kinds of datasets.
Besides empirical study, there are also theoretical investigation on the expressive power of neural networks, which mainly focuses on studying the number of layers and hidden units while choosing some commonly used activation function \cite{AroraBMM18}. As we can see, current research explores little about the theoretical property of activity function, which also contributes to the expressive power of neural network. Our study fills this gap by studying how the activation function reduces the neural network loss. 
