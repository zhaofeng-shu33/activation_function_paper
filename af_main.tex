...
Besides empirical study, there are also theoretical investigation on the expressive power of neural networks, which mainly focuses on studying the number of layers and hidden units while the activation function is fixed to some commonly used one \cite{AroraBMM18}. Current research explores little about the theoretical property of activity function, which also contributes to the expressive power of neural network. Our study fills this gap by studying how the activation function influences the network loss.
...
To simplify the analysis, we consider the simplest neural network with only one output node and several input node, which can be regarded as a simplified
ResNet model.
...
Our measure of activation function is not restricted to one specific input and output pair but is averaged over the pair sampled from some distributions.
