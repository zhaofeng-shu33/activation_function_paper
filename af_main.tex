...
we choose $\bm{X}$ as uniformly distributed random orthogonal matrix \cite{eaton1989group}.
The choice of $\bm{X}$ can be regarded as the post-processing result of PCA (Principal Component Analysis):
If each element of $\bm{X}'$ is i.i.d. $\mathcal{N}(0, 1)$ random variables and $\sigma(\bm{X}'\underline{w}')$ is considered,
we can make the transformation $\bm{X} = \bm{X}'(\bm{X}'^\T\bm{X}')^{-1/2}, \underline{w} = (\bm{X}'^\T\bm{X}')^{1/2}\underline{w}'$.
