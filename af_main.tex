...
\author[2]{Shao-Lun Huang\corref{cor1}}
\ead{shaolun.huang@sz.tsinghua.edu.cn}

\cortext[cor1]{Corresponding author}

\affiliation[1]{organization={Department of Electronics Engineering, Tsinghua University},
postcode={100089},
city={Beijing},
country={China}
}

\affiliation[2]{organization={Tsinghua-Berkeley Shenzhen Institute},
postcode={518000},
city={Shenzhen},
country={China}
}
...
%%
\begin{abstract}
While non-linear activation functions play vital roles in artificial neural networks, it is generally unclear how non-linearity can improve the quality of function approximations.
...
In addition to empirical study, there are also theoretical results
which investigate specific network architectures.
Many researches are restricted on a specific activation function and seldom explore the theoretical property of activation function,
...
Our study fills the theoretical gap by studying quantitatively
how the activation function reduces the neural network loss.

In particular, our work is not restricted to specific family of functions but only requires the activation function to be 
a perturbation of linear functions.
This assumption allows us to study the non-linear gain when
non-linear activation function is added to the network.
Such a gain can be expressed quantitatively up to second order approximation.
To obtain non-trivial insight while simplify the expression,
...
are defined as a series of polynomials satisfying the orthogonal property $ \E[H_m(X)H_n(X)] = n! \mathrm{1}_{m=n}$ where $X$ is the standard normal random variable.
...
The research of Shao-Lun Huang is supported in part by the Shenzhen Science and Technology Program under Grant KQTD20170810150821146,
National Key R\&D Program of China under Grant 2021YFA0715202 and High-end Foreign Expert Talent Introduction Plan under Grant G2021032013L.
%%%%%%
