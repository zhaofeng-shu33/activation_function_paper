...
$x, \underline{x}, \bm{x}$ (or $\mathbf{M}$) represents numerical value,
numerical vector
and numerical matrix.
Besides, $\mathbf{I}_n$ denotes n-dimensional identity matrix, and
$[\cdot]^\T$ is the transpose operation on matrix. What's more, $\ide{i}{j} \triangleq (i+j)\, \mathrm{mod} \, 2$
...
This type of network arises naturally from function approximation problems, where a target function $g$, defined in a finite set,
...
For a given sampling, we have $n$ data pairs $(x_1, y_1), \dots, (x_n, y_n)$, and
...
From \cite[Proposition 7.1]{eaton1989group} $\bm{X}$ is an orthogonal matrix, uniformly distributed in the orthogonal group.
