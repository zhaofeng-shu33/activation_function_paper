...
In artificial neural networks, activation functions are applied to linear combinations of input features and
enhance performance of the whole system.
The current approach to choose
activation function is mostly empirically motivated.
For example, it is a common practice to optimize over parameterized activation function to obtain desirable activation function \cite{ramachandran2017searching}.
However, to balance the training efficiency and network performance, the family of activation functions should be carefully chosen \cite{laudani2015training}. In addition, the searched activation function is not necessary the optimal one. These two characteristics limits the effectiveness of empirical study.
