...
A good neural network requires not only the careful choice of the inner structure of each layer but designing the layer to layer activation. The current approach to choose
activation function is mostly empirically motivated.
For example, it is a common practice to optimize over parameterized activation function together with network weight to obtain desirable activation function \cite{ramachandran2017searching}.
However, to balance the training efficiency and network performance, the family of activation functions cannot be too complex \cite{laudani2015training}. The searched activation
function is not necessary the optimal one. These two characteristics limits the effectiveness of empirical study.
Besides empirical study, there are also theoretical investigation on the expressive power of neural networks, which mainly focuses on the power of the number of layers and hidden units while the activation function is fixed to some commonly used one \cite{AroraBMM18}. Current research explores little about the theoretical property of activity function, which also contributes to the expressive power of neural network. Our study fills this gap by studying how the activation function influences the network loss.
...
By doing so, we can compare the non-linear loss with that of linear network.
To simplify the analysis, we consider the simplest neural network with only one output node and several input node.
...
To the best of our knowledge, this is the first work that can explicitly quantify the performance gain in functional approximation, and provide the close form solutions of the optimal non-linear activation functions for certain classes of neural networks.
