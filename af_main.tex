...
Many researches are restricted on a specific activation function and seldom explore about the theoretical property of activity function,
which is important to the expressive power of neural network.
For example, \cite{AroraBMM18} focuses on studying the number of layers and hidden units while choosing ReLU activation function.
...
mathematically.
Under some assumptions we establish the optimal solution as the Hermite polynomial and discover the expression of error rate in Section \ref{sec:hp}.
Furthermore, in Section \ref{sec:er}, numerical experiments are conducted to verify our theoretical results.
Section \ref{sec:con} concludes the paper. Proof details of our conclusion are provided in \ref{sec:Proofs}.

Throughout this paper,
we use $X,\underline{X}, \bm{X}, $ to represent random variables,
random vectors and
random matrix. In addition,
$x, \underline{x}, \bm{x}$ (or $\mathbf{M}$) represents a numerical value,
a numerical vector
and numerical matrix.
Besides, $\mathbf{I}_n$ is n-dimensional identity matrix, and
$[\cdot]^\T$ is the transpose operation on matrix. What's more, $\ide{i}{j} = (i+j)\, \mathrm{mod} \, 2$
...
To be more specific, $Y$ is approximated using $ \sigma(\sum_{i=1}^k w_i X_i)$.
...
For different problems, we suppose $\underline{y}, \bm{x}$ are drawn from some distribution $G$.
...
To extend our result on general non-linear functions, we need to choose a proper function space $\mathcal{F}$.
By detailed analysis we consider a local regime which contains functions near to linear function.
The distance between non-linear and linear function is quantified by a positive value $\epsilon$.
Thus we consider $\mathcal{F}_{\sigma_0}(\epsilon) =\{ \sigma \,|\, \norm{\sigma - \sigma_0} \leq \epsilon\}$.
Then all differential functions can be treated as $\cup_{\epsilon>0} \mathcal{F}(\epsilon)$.
The norm is actually the expectation w.r.t. the distribution space $G$.
...
when $\sigma$ is contracted to $\sigma_0$ along a certain path in $\mathcal{F}(\sigma)$.
...
Our goal is to get the fastest decreasing path from $E[\sigma_0]$ to $E[\sigma]$, this is equivalent to solve an optimization problem $\min \mathrm{C}[\xi]$ constraint by $\xi \in \mathcal{F}_0$. It is hard to optimize on $\mathcal{F}_0$ directly, and we consider $\mathcal{F}_{0,m} = \mathcal{F}_0 \cap P_m$ instead. $P_m$ consists of polynomials with the highest degree no greater than $m$, and we have $\mathcal{F}_0 = \lim_{m\to \infty} \mathcal{F}_{0,m}$.
