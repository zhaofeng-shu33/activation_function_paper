...
Suppose $m << k$, then we have 
\begin{align}
\mathrm{C}[\sigma] &= (1-\frac{k}{n}) q^T M q \\
\textrm{where }q_i &= \frac{k^{i/2}}{n^{- 1/2 + i}}p_i \\
M_{ij} =& -\delta(i,j)(i-1)(j-1)(i+j-3)!! 
\end{align}
and the normalization constraint from Equation \eqref{eq:ug} is
$p^*$ is the solution to the following quadratic optimization problem:
\begin{align}
\min \, & p^T M p \\
s.t. \,& p^T N p = 1 \\
N_{ij} =& \delta(i,j)(i+j-1)!!
\end{align}
Then the universally good activation function satisfies: 
\end{proposition}
