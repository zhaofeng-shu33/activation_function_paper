...
Using Equation \ref{formularC123}, we have
\begin{align*}
    \bm{M}_{ij} &= n(1-\bm{A}_{11}) (\E[Z_1^{i+j}] -
    ij \frac{1}{n}\bm{A}_{11} \E[Z_1^{i+j-2}])  \\
    &-n(n-1)\bm{A}_{12}(\E[Z_1^i Z_2^j] - ij \frac{1}{n}\bm{A}_{12}\E[Z_1^{i-1}Z_2^{j-1}])
\end{align*}
We can approximate $\E[Z_1^{2t}]$ where
$Z=\bm{A}Y$ assuming $Y$ is a Gaussian vector with variance $\frac{1}{n}$.
Then $\E[Z_1^{2t}] = \frac{1}{n^t}\bm{A}_{11}^t (2t-1)!!$.
\end{proof}
