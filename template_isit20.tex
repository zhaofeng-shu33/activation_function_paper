...
It says the local decreasing rate of $\E[\sigma]$ is linearly improved if we increase the degree of polynomials. The bound can be reached if $ m << k$ and generally $-(m-1) < C[\xi]$ if the assumption $ m << k$ fails.
...
+ \epsilon \underline{\hat{w}} + o(\epsilon)$ where $\underline{w}_0$ is the minimizer for
