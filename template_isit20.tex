...
As shown in Figure \ref{fig:ns}, random variable $Y$ is approximated by $ \sigma(\sum_{i=1}^k w_i X_i)$ where $\sigma(z) = z + \epsilon \xi(z)$. For a given problem, the number of sample $n$ is given and the L2 loss function is given by
$ \norm{\bar{Y} - \sigma(\sum_{i=1}^k w_i \bar{X}_i)}^2 $ where $\bar{X}_i$ is the feature vector for the i-th dimension and $\bar{Y}$ is the probability vector for classification problem or the fitted value for the regression problem.
