...
\textrm{where }\underline{p}_i &= \frac{k^{i/2}}{n^{- 1/2 + i}}\underline{q}_i \notag\\
...
Let
\begin{equation}
    \xi_m(z) = \frac{1}{\sqrt{mn}} H_m(\frac{n}{k^{1/2}} z)
\end{equation}
...
Let $\mathbf{U}$ be an $(m+1) \times (m+1)$ upper trianglar matrix defined by
$U_{ij} = \delta(i, j) \frac{j!}{(j-i)!!\sqrt{i!}} \textrm{ for } 0 \leq i\leq j \leq m$, $\underline{e_m}$ is an $(m+1)$ dimensional vector whose elements are zero except that the last element is 1. $\underline{p}_i = \delta(i, m) (-1)^{(m-i)/2}\frac{m!}{i!(m-i)!! \sqrt{m!}}$.
...
\begin{proof}[Proof of Lemma \ref{lemma:Upem} ]
    ...
    \sum_{j=i}^m \delta(i, j) \frac{j!}{(j-i)!!\sqrt{i!}}  \frac{\delta(j, m)(-1)^{(m-j)/2}m!}{j!(m-j)!! \sqrt{m!}} = 0
    \end{equation}
    ...
    \begin{align*}
    \sum_{j=i, \delta(i,j)=1}^m (-1)^{(m-j)/2} \frac{1}{(m-j)!!(j-i)!!}  = 0 & \iff \\ 
    \sum_{j=0, j \textrm{ is even}}^{2m'} (-1)^{(2m'-j)/2} \frac{1}{(2m'-j)!! j!!} = 0 & \iff \\
