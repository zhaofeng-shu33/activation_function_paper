...
However, theoretical property is also important as a gurantee to the functionality of activation function in neural network.
In this paper, we study how the activation function influences the network loss
when the activation term is contrained to a small perturbation of linear function.
...
In Section \ref{sec:hp}, the Hermite polynomial solution for our problem is briefly deducted.
...
$\mathrm{E}[\sigma]$ is called averaged estimation error whose definition is
...
We restrict our discussion of universally good activation function to local regine analysis to make the analytic solution possible.
...
\mathrm{C}[\sigma]\epsilon^2 + o(\epsilon^2)$
where
\begin{align}\notag
...
\begin{proposition}
We consider
$\xi(z) = \sum_{i=0}^m a_i z^i$, and $\underline{q} = [a_0, a_1, \dots, a_m]^T$.
The universally good activation function in $P_m(z)$ is the solution to
\begin{align}
a
\end{align}
\end{proposition}
